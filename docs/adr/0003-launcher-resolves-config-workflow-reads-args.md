...
- The **`/bucket:run` slash command (the Launcher)** is a prompt. It reads `bucket.config.json`, resolves the active Preset, and invokes the `Workflow` tool, passing the fully-resolved config in as `args`.
...
- Invoking `/bucket:run` is also the user opt-in the Workflow tool requires before spawning agents — a single, intentional entry point.
